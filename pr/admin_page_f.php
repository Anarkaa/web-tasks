    ...
    $dbhost = 'localhost';
         ...
         if(! $conn ){
          ...
          if(isset($_GET['delete'])){
             $sql = "DELETE FROM f WHERE ID=".$_GET['delete'];
           $result = mysqli_query($conn,$sql);
         }
            
         if (isset($_GET['Image'])) {
            $sql = "INSERT INTO f (ID, Image, Name, Price)
...
<html>
  ...
  <?php
    ...
    <div class="">
      ...
      <div class=""><a href="admin_page_f.php?delete='.$row['ID'].'">Delete</a></div>
      </div>
    </div>
        </div>';
    }
    ...
    mysqli_close($conn);
?>
  <form action="admin_page_f.php" style="margin-right: 50px">
      <h1>Admin Page</h1>
      <table>
            <tr>
                <td>Image(URL): </td>
